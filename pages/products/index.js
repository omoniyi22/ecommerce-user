                ...
                {allProducts?.length > 0 && allProducts.map(product => (
                    ...
                    border-opacity-10">
                            <div className="relative md:h-[300px] h-[200px]">
                                ...
                                object-contain opacity-0 group-hover:opacity-100" />
                            </div>

                            <div className="relative p-3 border-t">
                                <Link href={"/products/" + product._id}>
                                    <h3 className="text-md text-text group-hover:underline line-clamp-2">
                                        {product.title}
                                    </h3>
                                </Link>

                                <div className="mt-1 flex flex-col items-center justify-between text-text">
...
export async function getServerSideProps() {
    ...
    let allProducts = await Product.find({}, null, {
        sort: {
            id: 1
        }
    })
